...
Tekstredaktilo/
├── include/
│   ├── common.h        # Shared types: Operation, VectorClock, Role
│   ├── ot_engine.h     # OT + CRDT API
│   ├── auth.h          # Role-based auth API
│   ├── storage.h       # File locking storage API
│   └── logger.h        # Named pipe IPC API
├── src/
│   ├── ot_engine.c     # Vector clocks + OT transform engine
│   ├── auth.c          # User table and permission checks
│   ├── storage.c       # fcntl locking + atomic file save
│   ├── logger.c        # FIFO writer/reader shared library
│   ├── server.c        # TCP server, pthreads, mutex, semaphore
│   ├── client.c        # ncurses terminal UI + socket client
│   └── logger_proc.c   # Standalone logger process (IPC reader)
├── data/
│   └── document.txt    # Persisted document (auto-created)
├── logs/
│   └── server.log      # Log output (auto-created by logger)
└── Makefile
```
